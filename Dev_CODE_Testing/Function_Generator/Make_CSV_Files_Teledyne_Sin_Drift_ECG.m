...
Signal_ECG.signal = Signal_vec(1:(round(length(Signal_vec)/Parameter.Function_generator_fs)):length(Signal_vec));
Signal_ECG.signal = Signal_ECG.signal-min(Signal_ECG.signal);
Signal_ECG.signal = Signal_ECG.signal./max(Signal_ECG.signal);
Signal_ECG.signal = repmat(Signal_ECG.signal,ceil(length(Signal_vec)/Parameter.Function_generator_fs),1)
Signal_ECG.signal = (1/Parameter.ECG_Amp_Signal_SNR )*Parameter.Signal_Amp*Signal_ECG.signal(1:length(Signal_vec))';
...
Signal_sin.signal   = Parameter.Signal_Amp * sin(Signal_sin.f *Signal_sin.timevec*2*pi) ;
Signal.signal       = Signal_sin.signal+Signal_Jumps.signal + Signal_ECG.signal ;
